...
class IMATAC(nn.Module):
    ...
    def shut_dropout(self):
        dropout_names = [name for name, module in self.named_modules() if isinstance(module, nn.Dropout)]
    ...
    def imputation(self, dataloader, device, output_scale='log'):
        output = pd.DataFrame()
        print('Processing...')
        raw_shape = getattr(dataloader.dataset, 'raw_shape', None)
        for x, _, b, _ in dataloader:
            ...
            if raw_shape is not None:
                dec = dec[..., :raw_shape]
            if output_scale == 'raw':
                dec = torch.pow(2, dec * torch.log2(torch.tensor(100.0, device=dec.device))) - 1
